"""Tests for cost increase detection."""

import pytest


def test_scan_increases(client, auth_header):
    r = client.post("/api/scan", headers=auth_header)
    assert r.status_code == 200
    d = r.get_json()
    assert "increases_found" in d


def test_list_increases(client, auth_header):
    r = client.get("/api/increases", headers=auth_header)
    assert r.status_code == 200
